...
class SegmentType(str, Enum):
    ...
    grounding = "grounding"
    deepening = "deepening"
    # Guidance segment type - used for providing mindful awareness instructions
    # and present moment consciousness guidance during meditation
    guidance = "guidance"
...
class MusicParameters(ActionParameters):
    ...
    fade_duration: Optional[int] = Field(
        None, description="Duration of fade in/out in seconds"
    )
...
class MeditationStructure(BaseModel):
    ...
    background_music_style: Optional[str] = Field(
        None, description="Suggested music style if any"
    )
    planned_segments: List[Dict[str, Any]] = Field(
        ..., description="Outline of planned segments with approximate timing"
    )
    key_elements: List[str] = Field(
        ..., description="Important elements to include in the meditation"
    )
    breathing_pattern: Dict[str, Any] = Field(
...
class VoiceInstruction(BaseModel):
    ...
    text: str = Field(..., description="The text to be spoken")
    placement: str = Field(
        ..., description="When this instruction occurs (beginning, middle, end)"
    )
...
class BreathGuidance(BaseModel):
    ...
    cues: Optional[Dict[str, str]] = Field(
        None, description="Optional verbal cues for breath phases"
    )
...
class ScriptSegment(BaseModel):
    ...
    timing_description: str = Field(
        ..., description="Timing description (e.g., '00:00-01:30')"
    )
    voice_instructions: List[VoiceInstruction] = Field(
        ..., description="Voice instructions with text and placement"
    )
    breath_guidance: Optional[BreathGuidance] = Field(
        None, description="Breath guidance if applicable"
    )
    silence_periods: Optional[List[Dict[str, Any]]] = Field(
        None, description="Silence periods if applicable"
    )
    background_music: Optional[Dict[str, Any]] = Field(
        None, description="Background music instructions if applicable"
    )
...
class MeditationContent(BaseModel):
    ...
    segments: List[ScriptSegment] = Field(
        ..., description="List of all meditation script segments"
    )
    
    @validator("segments")
    def validate_segment_types(cls, segments):
        """
        Ensure all segment types are valid according to the current SegmentType enum
        """
        # Verify that each segment type is a valid SegmentType value
        valid_segment_types = {segment_type.value for segment_type in SegmentType}
        for segment in segments:
            if segment.segment_type.value not in valid_segment_types:
                raise ValueError(f"Invalid segment_type: {segment.segment_type}")
        return segments
...
class TimedInstruction(BaseModel):
    ...
    content: Optional[str] = Field(
        None, description="Content of instruction (e.g., voice text)"
    )
    parameters: Union[
        SpeakParameters,
        PauseParameters,
        ...
        SilenceParameters,
        MusicParameters,
    ] = Field(..., description="Type-specific parameters for this instruction.")
...
class TimedSegment(BaseModel):
    ...
    instructions: List[TimedInstruction] = Field(
        ..., description="Timed instructions within this segment"
    )
...
class MeditationTiming(BaseModel):
    ...
    total_duration_seconds: int = Field(
        ..., ge=1, description="Total duration in seconds"
    )
    ...
    def check_duration_matches_segments(cls, v, values):
        ...
        if "segments" in values and values["segments"]:
            ...
            if v != max_end_time:
                raise ValueError(
                    f"Total duration ({v}s) does not match end of last segment ({max_end_time}s)"
                )
        return v
...
class Action(BaseModel):
    ...
    start_time_seconds: int = Field(
        ..., ge=0, description="Start time of the action in seconds."
    )
    duration_seconds: int = Field(
        ..., ge=0, description="Duration of the action in seconds."
    )
    parameters: Union[
        SpeakParameters,
        PauseParameters,
        ...
        SilenceParameters,
        MusicParameters,
    ] = Field(..., description="Type-specific parameters for this action.")
...
class Segment(BaseModel):
    ...
    start_time_seconds: int = Field(
        ..., ge=0, description="Start time of the segment in seconds."
    )
    end_time_seconds: int = Field(
        ..., ge=1, description="End time of the segment in seconds."
    )
...
class MeditationSession(BaseModel):
    ...
    title: str = Field(..., description="Title of the meditation session.")
    duration_seconds: int = Field(
        ..., ge=1, description="Total duration of the session in seconds."
    )
    ...
    background_music: Optional[str] = Field(
        None, description="Background music for the session, if any."
    )
    ...
    def check_duration_matches_segments(cls, v, values):
        ...
        if "segments" in values and values["segments"]:
            ...
            if v != max_end_time:
                raise ValueError(
                    f"Duration ({v}s) does not match end of last segment ({max_end_time}s)"
                )
        return v
